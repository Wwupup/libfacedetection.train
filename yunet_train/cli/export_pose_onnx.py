...
def export_pose_onnx(args: argparse.Namespace) -> Path:
    ...
    model = build_yunet_pose(variant, kpt_shape=kpt_shape)
    ...
    if args.verify:
        _verify_onnx(model, example_input, output_file, kpt_shape=kpt_shape)
    return output_file
...
def _default_output_file(
    ...
    return Path("work_dirs") / "export" / f"{checkpoint.stem}_{variant}_pose_{tag}.onnx"
...
def _verify_onnx(
    ...
    *,
    ...
    with torch.no_grad():
        torch_outputs = [output.detach().cpu().numpy() for output in _flatten_export_outputs(model, example_input, kpt_shape)]
